---
title: 用 Assert 和 Suggest 断言优化 DSPy 应用
...
DSPy Assertions 是引导你的 LLM 应用朝着期望结果迭代的**路标和导航**。断言可以提升结果质量，并确保返回值的可预测性。

### 什么时候使用断言

你需要亲手捏出结果的形状，而不只是满足于评判结果的好坏，`Assert` 和 `Suggest` 断言就会十分好用。

更加严谨地表述是，当单纯靠[指标评估](https://www.oldcai.com/ai/dspy-evaluation/)不足以训练出满足复杂的要求的应用；或单凭 `metric` 的随机结果排序来优化，需要巨大的成本才能训练出符合要求的训练结果，就需要使用断言。
