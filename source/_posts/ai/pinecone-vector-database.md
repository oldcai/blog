---
title: 教程:基于语义PDF搜索、对话的大模型RAG应用,从0开始
...
通过创建一个`PyPDFLoader`加载器来加载这份PDF文件，并通过`loader.load()`方法读取数据。如果你的PDF文件是非结构化的或者你想从网络上加载PDF，也可以使用`UnstructuredPDFLoader`或`OnlinePDFLoader`。

接着，我们可以检查上传文件包含的文档和字符数量。如果你使用的是`PyPDFLoader`，文本应该已经被按页分割。
...
在Pinecone的免费试用版中，用户可以创建一个索引，正好可以适合我们折腾。
